...
compatibility: Requires terraform, msgraph provider ~> 0.5, and azuread provider ~> 3.0
---
...
Use [Microsoft policy update permissions](https://learn.microsoft.com/en-us/graph/api/authenticationmethodspolicy-update?view=graph-rest-1.0) and the specific method endpoint for evidence.
`UserAuthenticationMethod.ReadWrite.All` is not a blanket requirement for tenant policy configuration. Verify read and write operations separately.
...
- `msgraph_resource.auth_method_policy_verifiable_credentials`
- `msgraph_resource.auth_method_policy_qr_code_pin`
## Guardrails
