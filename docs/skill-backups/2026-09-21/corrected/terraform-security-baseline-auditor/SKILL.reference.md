...
- Prefer a typed `azuread_*` resource when the AzureAD provider supports the resource and required properties. Use `msgraph_resource` only for Microsoft Graph APIs without an AzureAD equivalent. When changing an existing resource type, prepare a reviewed state migration before deployment; execute it only with authorization.
...
- Preserve adoption intent for existing objects. Use the provider-documented import identifier, which may differ from the configured collection URL.
  For example, a `url = "policies"` resource can import `policies/authenticationMethodsPolicy`.
  Beta imports require the provider-supported API-version suffix. Do not assume every resource is a singleton or requires import.
  Newly created groups, authentication strengths, and catalogs differ from existing tenant policies.
  Prepare and review state migration before a resource-type change; execute it only with authorization.
...
- Confirm HCL conventions are followed (no `jsonencode` around the entire Graph body (nested JSON-string properties may require it), adoption handled where needed, `depends_on` only where needed).
