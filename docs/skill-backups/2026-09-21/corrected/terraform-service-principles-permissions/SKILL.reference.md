...
compatibility: Requires terraform, azuread provider ~> 3.0, and msgraph provider ~> 0.5
---
...
- `Application.ReadWrite.OwnedBy` when the Terraform identity owns the application, or `Application.ReadWrite.All`.
- The current application has no configured owners; do not assume the ownership-scoped option works without an ownership decision.

See the provider documentation for [applications](https://registry.terraform.io/providers/hashicorp/azuread/latest/docs/resources/application) and [federated credentials](https://registry.terraform.io/providers/hashicorp/azuread/latest/docs/resources/application_federated_identity_credential).
Requested Maester permissions are separate from Terraform execution permissions. Declaring requested permissions does not grant consent.
Do not require `AppRoleAssignment.ReadWrite.All` unless the workflow actually grants app roles; neither resource here does so.
